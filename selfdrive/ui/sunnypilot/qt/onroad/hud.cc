...
void HudRendererSP::updateState(const UIState &s) {
  ...
  //自訂動態圖形(縱向+橫向)
  show_steer_state = s.scene.show_steer_state;
  show_long_state = s.scene.show_long_state;
  ...
  SteerWidget->setValue(enabled, latActive, -angleSteers, -CC_Steer);

  LongWidget->updateState(s);
  LongWidget->setValue(enabled, longActive, aEgo, CC_Long);
}
