...
Ported from the "carrot" openpilot fork's `selfdrive/carrot/traffic_stop.py`.
These are the pure, stateless distance-correction helpers used by
`TrafficStopController` (see `traffic_stop_controller.py`) to turn a
model-predicted stop-line distance into the position of a virtual,
stationary MPC obstacle placed at the stop line.
"""
